...
def get_dmname_from_majorminor(diskstats=False, major=0,minor=0):
    for disk in diskstats:
        if int(diskstats[disk]['minor']) == minor and int(diskstats[disk]['major']) == major:
            return(disk)
    return(False)
...
sector_size=512
xenstore_vbd = subprocess.Popen("/usr/sbin/xenstore-ls ", shell=True, bufsize=8192, stdout=subprocess.PIPE).stdout.readlines()
xmlist = subprocess.Popen("/usr/sbin/xm list -l", shell=True, bufsize=128000, stdout=subprocess.PIPE).stdout.readlines()
...
for server in vservers:
    for device in vservers[server]:
        ...
        dmname=get_dmname_from_majorminor(diskstats,int(major,16),int(minor,16))
