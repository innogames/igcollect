...
for node in nodes:
    ...
    for line in cat(
        '/sys/devices/system/node/node{i}/numastat',
            i=node).split('\n'):
...
for (node, node_stats) in total_stats.items():
    for (key, value) in node_stats.items():
        print('servers.{host}.system.numa.node{node}.{key} {value} {now}'.format(
            host=hostname, node=node, key=key, value=value, now=now()))
...
if conn is not None:
    ...
    for (guest, assignment) in result.items():
        for (node, num_vcpu) in assignment.items():
            print(
                'servers.{host}.virtualisation.vserver.{guest}.numa.node{node}.vcpu_count {value} {now}'.format(
                    host=hostname,
                    guest=guest.replace(
                        '.',
                        '_'),
