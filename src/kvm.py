...
for ID in domids:
    ...
    for vcpu in dom.vcpus()[0]:
        ...
        message += 'servers.{0}.virtualisation.vserver.{1}.vcpu.{2}.time {3} {4}\n'.format(
            hostname, name, vcpu[0], cputime, now)
        total_cpu += cputime
    message += 'servers.{0}.virtualisation.vserver.{1}.vcpu.time {2} {3}\n'.format(
        hostname, name, total_cpu, now)
