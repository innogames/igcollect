...
graphite_prefix = 'servers.' + socket.gethostname().replace('.', '_') + \
    '.software.rabbitmq'
...
for metric in overview_object_totals_metrics:
    print(graphite_prefix + '.object_totals.' + metric + ' ' +
          str(data['object_totals'][metric]) + ' ' + str(now))

for metric in overview_message_stats_metrics:
    print(graphite_prefix + '.message_stats.' + metric + ' ' +
          str(data['message_stats'][metric]) + ' ' + str(now))

data = download(rabbit_url + '/nodes/' + nodename)

for metric in nodes_metrics:
    print(graphite_prefix + '.' + metric + ' ' +
          str(data[metric]) + ' ' + str(now))
